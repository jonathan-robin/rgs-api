...
export function get_info_joueur(id, callback){ 
    con.query(`SELECT nom, prenom, nat, idjoueur from joueur where idjoueur = ${id}`, function(err, res, field) {
        if (err) throw err; 
        return callback(res);
    })
}
  ...
  */
  export function get_loose(id_joueur, callback){
    con.query(`SELECT * FROM score where IDJ1 = ${id_joueur} AND VAINQ != ${id_joueur} OR IDJ2 = ${id_joueur} AND VAINQ != ${id_joueur}`, function(err, result, field){
        if (err) throw err;
  ...
  */
  export function get_specific_loose(currentId, mappedId ,callback){
    con.query(`SELECT * FROM score where IDJ1 = ${currentId} AND IDJ2 = ${mappedId} AND VAINQ != ${currentId} OR IDJ2 = ${currentId} AND IDJ1 = ${mappedId} AND VAINQ != ${currentId}`, function(err, result, field){
        if (err) throw err;
        return callback(result)
    })
}
